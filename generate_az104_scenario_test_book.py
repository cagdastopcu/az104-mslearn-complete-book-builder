...
CASE_QUESTION_COUNT = 5

STEM_TEMPLATES = [
    "Which action best aligns with the guidance in section '{section}'?",
    "Which choice is most consistent with the chapter guidance for '{section}'?",
    "Which option should the administrator choose based on '{section}'?",
    "Which response matches the recommended approach in '{section}'?",
    "Which statement best fits the requirement described in '{section}'?",
]

SCENARIO_TEMPLATES = [
    "Your organization is implementing Azure changes in the area of '{subtitle}'.",
    "You are the Azure administrator for a company rolling out controls for '{subtitle}'.",
    "A migration project is underway and the team is validating '{subtitle}'.",
    "An operations incident affects resources related to '{subtitle}'.",
    "During a governance review for '{subtitle}', you must provide a defensible recommendation.",
]
...
def _format_question_block(
    ...
    stem = STEM_TEMPLATES[chapter_question_no % len(STEM_TEMPLATES)].format(
        subtitle=subtitle, section=section
    )
    ...
    out: list[str] = []
    out.append(f"#### Question {chapter_question_no} (Case {case_id}, Q{question_in_case})")
    out.append(stem)
    out.append("")
    ...
    out.append(
        f"**Explanation:** The correct option matches a source statement under section label "
        f"'{section}' in this chapter."
    )
...
def build_test_book(source_md: Path, output_md: Path) -> None:
    ...
    lines: list[str] = []
    lines.append("# AZ-104 Scenario-Based Test Book")
    lines.append("")
    lines.append(
        "This scenario-based test book is generated only from the latest AZ-104 source book content. "
        "Each chapter contains case studies with shared scenario context and linked MCQs. "
        "Each case has 5 linked questions, and each chapter has at least 15 questions total. "
    ...
    for chapter_idx, (title, chapter_lines) in enumerate(chapters, start=1):
        ...
        ordered_correct = [facts[i % len(facts)] for i in range(question_count)]
        case_count = (question_count + CASE_QUESTION_COUNT - 1) // CASE_QUESTION_COUNT

        chapter_qn = 1
        for case_idx in range(case_count):
            case_id = case_idx + 1
            start = case_idx * CASE_QUESTION_COUNT
            end = min(start + CASE_QUESTION_COUNT, question_count)
            case_questions = ordered_correct[start:end]

            context_pool = [f for f in facts if f not in case_questions]
            if len(context_pool) >= 3:
                context_facts = rng.sample(context_pool, k=3)
            else:
                context_facts = (context_pool + facts)[:3]
            scenario_intro = SCENARIO_TEMPLATES[case_idx % len(SCENARIO_TEMPLATES)].format(subtitle=subtitle)
            context_lines = [_ensure_sentence(f.text) for f in context_facts[:3]]
            while len(context_lines) < 3:
                context_lines.append("The team must avoid introducing configuration drift across Azure resources.")

            lines.append(f"### Case Study {case_id}")
            lines.append("")
            lines.append(f"**Scenario:** {scenario_intro}")
            lines.append("")
            lines.append(f"**Current state:** {context_lines[0]}")
            lines.append("")
            lines.append(f"**Operational detail:** {context_lines[1]}")
            lines.append("")
            lines.append(f"**Constraint:** {context_lines[2]}")
            lines.append("")
            lines.append(
                f"**Objective:** Choose the best answers for the linked questions in this case, "
                f"based on chapter '{title}' guidance."
            )
            lines.append("")

            for local_idx, correct in enumerate(case_questions, start=1):
                correct_idx = facts.index(correct)
                ...
                block = _format_question_block(
                    case_id=case_id,
                    question_in_case=local_idx,
                    chapter_question_no=chapter_qn,
                    chapter_title=title,
                    subtitle=subtitle,
        ...
        coverage_rows.append(
            {
                ...
                "case_studies_generated": case_count,
                "all_facts_covered": question_count >= len(facts),
            }
        )

    output_md.write_text("\n".join(lines).strip() + "\n", encoding="utf-8")
    coverage_path = output_md.with_name("AZ-104_Scenario_Test_Book_Coverage.json")
    coverage_summary = {
        ...
        "total_case_studies_generated": sum(int(r["case_studies_generated"]) for r in coverage_rows),
        "case_question_count_target": CASE_QUESTION_COUNT,
...
def main() -> None:
    ...
    output_md = run_dir / "book" / "AZ-104_Scenario_Test_Book.md"
